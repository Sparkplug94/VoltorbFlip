...
class gameBoard: #voltorb flip board class, generates a full game of voltorb flip
    ...
    def select(self, probV = probV, prob2 = prob2, prob3 = prob3):  #encodes probability distribution for voltorbs, 3 tiles, and 2 tiles
        ...
        # if seed < probV:
        #     return 0
        # elif seed < probV + prob3:
        #     return 3
        # elif seed < probV + prob3 + prob2:
        #     return 2
        # else:
        #     return 1
        if seed < prob3:
...
#THIS METHOD SHOULD ONLY BE USED IF THE GAMEBOARD IS AVAILABLE AND KNOWN TO THE COMPUTER -- FOR ALGORITHM TESTING
def FOM2(game, labels): #calculate figure of merit matrix
    charRow, charCol = charSumCheckAuto(game, labels) #find characteristic sum of rows and columns
    rowVoltorbs = game.stats[1] #get number of voltorbs in rows
    colVoltorbs = game.stats[3] #get number of voltorbs in columns
    rowFom = charRow/(1+rowVoltorbs)  # calculate the figure of merit for each row
    colFom = charCol/(1+rowVoltorbs)  # calculate the figure of merit for each column
    fomMat = np.round(np.outer(rowFom,colFom),2) #take the outer product of these two vectors to create a matrix
    #set all elements with known values to 0 FOM
    for i in range(0,5):
        for j in range(0,5):
...
def FOMMaxPointsProbabilistic(game, labels): #Picks the square with the minimum probability of being a voltorb, using branching tree algorith to find probabilities
    stats = game.stats
    try:
        # print('\nTrying to construct all boards')
        solved_boards = branchBoards(stats, labels, [])
        ...
        for i, board in enumerate(solved_boards_int):
            ...
            if not checksum == 0:
                print('THIS BOARD IS INVALID (SOMEHOW)')
        fomBoard = 2*probabilityBoard[:, :, 3] + probabilityBoard[:,:,2]
        # set all elements with known values to 0 FOM
        for i in range(0, 5):
            for j in range(0, 5):
                if len(labels.map[i, j]) == 1:
...
def playVoltorbFlip(branching = 0, realBoard = 0): #play a game of voltorb flip, using the algorithm, and return success or failure
    ...
    for i in range(0,maxSteps):
        ...
        elif branching == 'maxpts':
            fom, _, _, naive = FOMMaxPointsProbabilistic(game, labels) #construct figure of merit matrix
        elif branching == 'minvoltorbs':
            fom, _, _, naive = FOMMinVoltorbsProbabilistic(game, labels) #construct figure of merit matrix
        elif branching == 'test':
            fom = FOM2(game, labels) #construct figure of merit matrix
        else:
